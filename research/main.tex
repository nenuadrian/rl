...
For this manuscript's LLM notation, the direct mapping is $s\leftrightarrow x$, $a\leftrightarrow y$, and $h=(x,y)$. Under that identification, VIREL is explicitly EM-like: the E-step performs policy improvement by reducing $\mathrm{KL}(q_\theta\|p_\omega)$, while the M-step performs policy evaluation via critic updates.
