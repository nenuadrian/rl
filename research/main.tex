...
Direct Preference Optimization (DPO) \parencite{rafailov2023dpo} is a preference MLE method: it bypasses reward modelling and policy-gradient estimation entirely, reducing alignment to a single binary cross-entropy objective over preference pairs.
...
\]
Expanding the KL and substituting \(\pi^\star\) \eqref{eq:awr_opt_policy} yields an equivalent supervised regression objective:
\[
  \pi_{k+1}
  \;=\;
  \arg\max_{\pi\in\Pi}\;
...
\]
Comparing with \eqref{eq:vmpo_form} we identify the V-MPO temperature as $\eta = t = \alpha+\beta$.
...
\begin{enumerate}
  ...
  \item \textbf{Engineered stabilisers.} DAR introduces practical stabilisers for
    ...
    importance-weight corrections). V-MPO implementations use top-$k$ and
    explicit dual updates which require numerical handling when lifted to LLMs.
\end{enumerate}

\paragraph{Bridge to the EM.}
Two views of the pattern: an E-step that
...
the local responsibility used in the paper's hard-assignment E-step reduces to the BT soft-likelihood:
\begin{equation}
  w(\phi_u; x,y_1,y_2)
  \;=\;
...
\paragraph{Why ratio-based methods break.}
PPO's clipped objective depends on the importance-sampling ratio $r_t(\theta) = \pi_\theta(a_t\mid s_t)/\pi_{\theta_{\mathrm{old}}}(a_t\mid s_t)$. If dropout is active during optimisation, three problems arise: (i)~the numerator $\pi_\theta$ becomes a random variable that fluctuates across forward passes even for fixed $\theta$, injecting noise into the narrow clip window $[1{-}\epsilon,\,1{+}\epsilon]$; (ii)~the denominator is computed once at rollout time in eval mode (no dropout), so the ratio conflates policy change with mask change, breaking the KL-constraining guarantee of clipping; (iii)~auxiliary KL penalties $\widehat{\mathrm{KL}} = \mathbb{E}_t[\ell_{\mathrm{old},t} - \ell_{\theta,t}]$ are biased upward by mask-induced variance, causing premature termination of updates.
...
\paragraph{Dropout compatibility.}
Because the M-step weights are detached from current parameters, dropout applies directly as standard regularisation (Section~\ref{sec:dropout_case}).
...
The table below compares every method discussed in this paper across all dimensions
relevant to the EM-vs-policy-gradient analysis.
Columns are ordered from pure policy-gradient (PPO) through collapsed EM (DPO) to
explicit two-phase EM methods (AWR, V-MPO, DAR), with GEMPI as the unifying framework.
...
\medskip
\noindent\textbf{Notes.}
\textit{Detached weights}: the per-sample weights $w_i$ entering the M-step are
computed with \texttt{stop\_gradient}, so gradients flow only through
$\log\pi_\theta$, not through the weights.
$\Lambda\!\coloneqq\!\sum_j\lambda_j$ is the effective temperature in the GEMPI E-step.
AT-DAR and DAR-TR are novel instantiations derived within GEMPI
(Section~\ref{sec:gempi_novel}).
\textit{BT}: Bradley--Terry preference model used in the DPO derivation.
...
For Atari, the shared policy-value network uses a convolutional ResNet backbone feeding the transformer core, with previous reward and action as additional inputs. Representative hyperparameters: embedding size 256, 8 layers, 4 attention heads, key/value size 32, MLP size 512, unroll length 63, batch size 128. Parameters are updated jointly under $\mathcal{L}(\phi,\theta,\eta,\alpha)$ using Adam at $10^{-4}$.
