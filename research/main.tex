...
PPO is a \emph{policy-gradient} method: it directly estimates and follows the gradient of expected return in parameter space. Specifically, it is an on-policy actor-critic algorithm that constrains each policy update to stay close to the behaviour policy via a clipped surrogate objective, avoiding the instability of unconstrained policy gradient steps.
...
\subsection{DPO as Preference MLE}
...
\subsection{DPO and the EM Framework}

\paragraph{Shared E-step form.}
...
\]
In a standard EM method (V-MPO, DAR, AWR), one would now \emph{construct} this target distribution and then project to $\pi_\theta$ via weighted MLE in the M-step.

\paragraph{The ``collapsed EM'' interpretation.}
DPO eliminates the explicit E-step entirely. Instead of constructing $q^\star$ and then fitting $\pi_\theta$ to it, DPO substitutes the \emph{functional form} of the optimal policy directly into the Bradley--Terry preference likelihood. The intractable partition function $Z(x)$ cancels in the reward difference $r(x,y_w) - r(x,y_l)$, yielding a closed-form objective that implicitly solves both the E-step and M-step in a single stage.
...
\begin{itemize}
  \item \textbf{Reward flexibility.} DPO's reward is \emph{defined} as $\beta\log(\pi_\theta/\pi_{\mathrm{ref}})$; it cannot incorporate richer reward signals (e.g.\ multi-aspect scores, process rewards) without redefining the objective.
...
The methods derived in this paper, together with PPO, fall into three structurally distinct paradigms for aligning language models. Making these paradigms explicit clarifies the design space and motivates the paper's focus on EM-based methods.

\subsection{Paradigm 1: Policy Gradient (PPO)}
...
\subsection{Paradigm 2: Preference MLE (DPO)}
...
\subsection{Paradigm 3: EM-Based (V-MPO, DAR, AWR---unified by GEMPI)}
...
\subsection{Key Structural Differences}
...
\paragraph{Sequence-level credit assignment.} Rewards are naturally sequence-level in LLM alignment; the EM weighting avoids unstable token-wise importance ratios.

\paragraph{Long-context batching.} Variable-length prompts and completions require masking and packed batches; the weighted M-step remains unchanged under standard attention masks.

\paragraph{Large-vocabulary stability.} Top-$k$ filtering and log-sum-exp normalisation reduce weight degeneracy and floating-point overflow in mixed-precision training.

\paragraph{Distributed training compatibility.} The objective is a weighted cross-entropy, so it integrates with gradient accumulation, FSDP/ZeRO, and standard transformer training pipelines.
