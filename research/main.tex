...
Consider RL fine-tuning on prompts $x\sim \mathcal{D}_x$ with reward $R(x,\tau)$ and a reference policy $\pi_{\mathrm{ref}}(\tau\mid x)$. A common objective is the KL-regularised expected reward
\begin{equation*}
  J(\theta) = \mathbb{E}_{x}\,\mathbb{E}_{\tau\sim \pi_\theta(\cdot\mid x)}
  \Big[ R(x,\tau) - \beta\,\mathrm{KL}\!\big(\pi_\theta(\cdot\mid x)\,\|\,\pi_{\mathrm{ref}}(\cdot\mid x)\big)\Big],
\end{equation*}
with $\beta>0$ controlling the trust region to $\pi_{\mathrm{ref}}$.

The E-step constructs an energy-based target distribution from rollouts and advantage estimates:
\begin{equation*}
  \text{(E-step)}\quad
  q(\tau\mid x) \propto \pi_{\mathrm{ref}}(\tau\mid x)\exp\!\left(\frac{R(x,\tau)}{\beta}\right).
\end{equation*}
...
\end{equation*}
optionally with an explicit constraint/penalty on $\mathrm{KL}(\pi_\theta\,\|\,\pi_{\mathrm{ref}})$ to stabilise optimisation.
...
\]
where $g(\eta)=\log(Z(\eta)/k)$ and $\varepsilon_\eta > 0$ is the target entropy/KL budget. Minimising $\mathcal{L}_\eta$ with respect to $\eta$ automatically tunes the temperature so that the E-step distribution does not collapse or spread too widely.
...
\]
Gradient updates are applied to $\xi$ via the chain rule $\nabla_\xi \mathcal{L}_\eta = (d\mathcal{L}_\eta/d\eta)(d\eta/d\xi)$.
...
\begin{itemize}
  ...
  \item \textbf{Log-sum-exp normalisation} (Eq.~\eqref{eq:seq_logsumexp}) addresses floating-point
